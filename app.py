...
import os # <-- 確保有導入 os 模組

# --- 字型設定 ---
# 獲取當前腳本的目錄 (通常是 Streamlit app.py 所在的目錄)
current_dir = os.path.dirname(__file__)

# 構建字型文件的絕對路徑
# 根據你提供的路徑 C:\Users\User\Desktop\Python3.8\01_haggingface\mlp\fonts\NotoSansTC-VariableFont_wght.ttf
# 假設 app.py 在 01_haggingface 內，那麼字型就在 01_haggingface/mlp/fonts/
font_path = os.path.join(current_dir,  "fonts", "NotoSansTC-VariableFont_wght.ttf")

# 檢查字型文件是否存在，並設定 Matplotlib 字型
if os.path.exists(font_path):
    from matplotlib.font_manager import FontProperties, findfont, findSystemFonts
    
    # 清除 Matplotlib 緩存，確保新字型被檢測到（有時需要，尤其是在部署環境）
    ...
    st.sidebar.success(f"已成功載入字型：`{os.path.basename(font_path)}`")
else:
    st.sidebar.warning(f"警告：字型文件未找到於 {font_path}。\n圖表中文可能顯示為方塊或亂碼。請確保字型文件放在 'mlp/fonts/' 資料夾中並已推送到 GitHub。")
    # 如果字型文件不存在，我們仍然保持預設字型設定，並警告使用者
    plt.rcParams['axes.unicode_minus'] = False # 仍可解決負號問題
